...
| `maven-context-runner` | Maven Context Runner | Run Maven goals (verify, test, clean, deep clean, compile, spotless:apply) on the module containing the current file. Deep clean runs `clean dependency:purge-local-repository` to also purge cached dependencies from ~/.m2. All commands except Spotless Apply, Compile, and Deep Clean run spotless:apply first. Shortcuts: Option+V, Option+T, Option+C, Option+D, Option+F, Option+B. |
